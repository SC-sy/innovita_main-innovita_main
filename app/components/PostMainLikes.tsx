...
export default function PostMainLikes({ post }: PostMainLikesCompTypes) {
    ...
    const sharePost = async () => {
        const shareData = {
            ...
            text: "Take a look at this creative post on Innovita.",
            url: `${window.location.origin}/post/${post?.id}/${post?.profile?.user_id}`
        };
        ...
        <>
            <div id={`PostMainLikes-${post?.id}`} className="relative mr-[75px]">
                <div className="absolute bottom-0 pl-2">
                    ...
                    <button onClick={sharePost} className="text-center">
                        <div className="rounded-full bg-gray-200 p-2 cursor-pointer">
                            <FaShare size="25"/>
                        </div>

                        <span className="text-xs text-gray-800 font-semibold">Share</span>
